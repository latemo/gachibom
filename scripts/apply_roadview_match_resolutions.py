"""Apply reviewed roadview uncertain-match resolutions to cards and open queue."""
...
from src.roadview_merge import apply_manual_match_resolutions, load_json, write_json


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    applied_at = date.fromisoformat(args.applied_at) if args.applied_at else date.today()
    result = apply_manual_match_resolutions(
    ...
    write_json(result["match_resolution_apply_report"], args.apply_report_output)

    summary = result["match_resolution_apply_report"]["summary"]
    ...
    print(
        ...
        f"resolved_uncertain_match_items:{summary['resolved_uncertain_match_items']}"
    )
    return 0
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--resolutions", type=Path, required=True, help="Reviewed uncertain-match resolutions JSON.")
    ...
    parser.add_argument("--apply-report-output", type=Path, required=True, help="Match-resolution apply report JSON.")
